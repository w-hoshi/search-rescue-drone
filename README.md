...
**[Demo 1 — mapping during flight](https://youtu.be/b8vK8jm33Qs)** — 2D occupancy grid building in real time (bottom right) as the drone flies an indoor corridor.

**[Demo 2 — person detection onboard](https://youtu.be/7SwgwFV0m5k)** — YOLOv8 running on the Pi 5 with the flight camera, drone held at a fixed height. This is a subsystem check that detection ran onboard at usable frame rate, not a demonstration of detection placed onto the map. The two demos are separate systems; see below.
...
**Autonomy was dropped.** `offboard_control` contains working PX4 offboard primitives (`takeoff_to_1m`, `forward_for_2s`) and nothing beyond them. Localization was not reliable enough to trust a closed-loop indoor mission, and the drone's marginal flight stability made it worse. All demo flights were manually piloted.
...
| Housing | PETG, 3D printed | CAD and photos in [`hardware/`](hardware/) |
...
Design constraints, the reasoning behind each of those choices, and what I would change are written up in [`hardware/README.md`](hardware/README.md).
